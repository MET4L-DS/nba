import { motion } from "framer-motion";

export function PageLoader() {
	return (
		<div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-white/70 dark:bg-gray-950/70 backdrop-blur-md overflow-hidden">
			{/* Top edge shimmer loading bar */}
			<div className="absolute top-0 left-0 w-full h-1 bg-gray-100 dark:bg-gray-900 overflow-hidden">
				<motion.div
					className="h-full bg-gradient-to-r from-indigo-500 via-violet-600 to-indigo-500"
					initial={{ x: "-100%" }}
					animate={{ x: "100%" }}
					transition={{
						repeat: Infinity,
						duration: 1.6,
						ease: "linear",
					}}
					style={{ width: "60%" }}
				/>
			</div>

			{/* Glowing central radial aura */}
			<div className="absolute w-[350px] h-[350px] rounded-full bg-indigo-500/10 dark:bg-indigo-500/5 blur-[80px] pointer-events-none" />

			{/* Central animated container */}
			<div className="flex flex-col items-center gap-6 relative z-10">
				{/* Logo frame with breathing pulsing and scale effects */}
				<motion.div
					className="w-20 h-20 rounded-2xl bg-white dark:bg-gray-900 border border-indigo-100 dark:border-indigo-950 flex items-center justify-center shadow-xl relative overflow-hidden group"
					animate={{
						scale: [1, 1.05, 1],
						boxShadow: [
							"0 10px 25px -5px rgba(99, 102, 241, 0.15)",
							"0 20px 25px -5px rgba(99, 102, 241, 0.3)",
							"0 10px 25px -5px rgba(99, 102, 241, 0.15)",
						],
					}}
					transition={{
						duration: 2.2,
						repeat: Infinity,
						ease: "easeInOut",
					}}
				>
					{/* Embedded shimmers */}
					<motion.div
						className="absolute inset-0 bg-gradient-to-r from-transparent via-indigo-500/10 to-transparent w-full"
						initial={{ x: "-100%" }}
						animate={{ x: "100%" }}
						transition={{
							repeat: Infinity,
							duration: 2,
							ease: "easeInOut",
						}}
					/>
					<img
						src="/tulogo.png"
						alt="Tezpur University Logo"
						className="w-14 h-14 object-contain drop-shadow-md select-none"
					/>
				</motion.div>

				{/* Text indicators with soft staggers */}
				<div className="text-center space-y-1.5">
					<motion.h3
						className="text-sm font-bold text-gray-900 dark:text-white tracking-wider uppercase bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-300 bg-clip-text"
						initial={{ opacity: 0, y: 8 }}
						animate={{ opacity: 1, y: 0 }}
						transition={{ type: "spring", stiffness: 350, damping: 25 }}
					>
						Tezpur University
					</motion.h3>
					<motion.div
						className="flex items-center justify-center gap-1"
						initial={{ opacity: 0 }}
						animate={{ opacity: 1 }}
						transition={{ delay: 0.2 }}
					>
						<span className="text-[11px] font-semibold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest animate-pulse">
							NBA OBE System
						</span>
					</motion.div>
				</div>
			</div>
		</div>
	);
}
